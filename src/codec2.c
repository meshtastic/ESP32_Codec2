...
void codec2_decode_700(struct CODEC2 *c2, short speech[],
                       const unsigned char *bits);
void codec2_encode_700b(struct CODEC2 *c2, unsigned char *bits, short speech[]);
void codec2_decode_700b(struct CODEC2 *c2, short speech[],
                        const unsigned char *bits);
...
struct CODEC2 *codec2_create(int mode) {
  ...
  assert((mode == CODEC2_MODE_3200) || (mode == CODEC2_MODE_2400) ||
         ...
         (mode == CODEC2_MODE_700) || (mode == CODEC2_MODE_700B));
  c2->mode = mode;
  for (i = 0; i < M; i++)
    c2->Sn[i] = 1.0;
  ...
  if (mode == CODEC2_MODE_700B)
    c2->gray =
        0; // natural binary better for trellis decoding (hopefully added later)
  else
    c2->gray = 1;
...
void codec2_encode(struct CODEC2 *c2, unsigned char *bits, short speech[]) {
  ...
  assert((c2->mode == CODEC2_MODE_3200) || (c2->mode == CODEC2_MODE_2400) ||
         ...
         (c2->mode == CODEC2_MODE_700) || (c2->mode == CODEC2_MODE_700B));
...
void codec2_decode_700(struct CODEC2 *c2, short speech[],
  ...
  for (i = 0; i < 4; i++) {
    ...
    aks_to_M2(c2->fft_fwd_cfg, &ak[i][0], LPC_ORD_LOW, &model[i], e[i], &snr, 0,
              0, c2->lpc_pf, c2->bass_boost, c2->beta, c2->gamma, Aw);
    apply_lpc_correction(&model[i]);
    synthesise_one_frame(c2, &speech[N * i], &model[i], Aw);
  }
...
void codec2_encode_700b(struct CODEC2 *c2, unsigned char *bits,
                        short speech[]) {
  MODEL model;
  float lsps[LPC_ORD_LOW];
  float mel[LPC_ORD_LOW];
  ...
  for (i = 0; i < 3; i++) {
    pack_natural_or_gray(bits, &nbit, indexes[i], lspmelvq_cb_bits(i),
                         c2->gray);
  }
...
void codec2_decode_700b(struct CODEC2 *c2, short speech[],
  ...
  for (i = 0; i < 3; i++) {
    indexes[i] =
        unpack_natural_or_gray(bits, &nbit, lspmelvq_cb_bits(i), c2->gray);
  }
