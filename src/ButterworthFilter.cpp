...
ButterworthFilter::ButterworthFilter(float frequency, int sampleRate,
                                     PassType passType, float resonance) {
  SetParameters(frequency, sampleRate, passType, resonance);
}

void ButterworthFilter::SetParameters(float frequency, int sampleRate,
                                      PassType passType, float resonance) {
  switch (passType) {
...
float ButterworthFilter::Update(float newInput) {
  float newOutput = a1 * newInput + a2 * inputHistory[0] +
                    a3 * inputHistory[1] - b1 * outputHistory[0] -
                    b2 * outputHistory[1];
