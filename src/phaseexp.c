...
// pexp->Wo_prev)/2.0; 	    float err = pred - model->phi[i]; 	    err =
...
static void predict_phases(struct PEXP *pexp, MODEL *model, int start,
                           int end) {
  int i;
...
// static void first_order_band(MODEL *model, int start, int end, float
// phi1_est)
// {
//     int   m;
...
// pexp->Wo_prev)/2.0; 		model->phi[i] = i*model->phi[1];
// 	    }
// 	    else
// 		model->phi[i] = PI*(1.0 - 2.0*(float)rand()/RAND_MAX); // note:
// try rand for higher harms 	    removed++;
// 	}
// 	else {
...
// static void limit_prediction_error(struct PEXP *pexp, MODEL *model, int
// start, int end, float limit)
// {
//     int   i;
...
// static void quant_prediction_error(struct PEXP *pexp, MODEL *model, int
// start, int end, float limit)
// {
//     int   i;
