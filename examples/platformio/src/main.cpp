...
enum RadioState { radio_standby, radio_rx, radio_tx };
...
ButterworthFilter hp_filter(240, 8000,
                            ButterworthFilter::ButterworthFilter::Highpass, 1);
...
int slapsed_encoder, slapsed_decoder, slapsed_tx, slapsed_ack, slapsed_rx,
    slapsed_rx_ack;
long start_tx;
...
void IRAM_ATTR onTimer() {
  ...
  if (radio_state == RadioState::radio_tx) {
    ...
    adc_buffer[adc_buffer_index++] =
        (16 * adc1_get_raw(ADC1_CHANNEL_0)) - 32768;

    // If you want to test with a 1KHz tone, comment the line above and
    // descomment the three lines below
    ...
    // Get a value from audio_fifo and convert it to 0 - 255 to play it in the
    // ADC If none value is available the DAC will play the last one that was
    // read, that's why the rx_raw_audio_value variable is a global one.
    if (audio_fifo.get(&v))
...
void run_codec2(void *parameter) {
  ...
  // Header, you have 4 bytes of header in each frame, yo can use it for you
  // protocol implementation
  //  for instace indicate address of the transmiter and address of the desire
  //  receiver, etc.
  ...
  while (1) {
    ...
    if (tcount != 0) // if the task was notified!
    {
      ...
      if (radio_state == RadioState::radio_tx) // Trasnmitting
      {
        ...
        codec2_encode(codec2_state, tx_encode_frame + tx_encode_frame_index,
                      speech);
      ...
      if (radio_state == RadioState::radio_rx) // Receiving
      {
        ...
        for (int i = ENCODE_FRAME_HEADER_SIZE; i < ENCODE_FRAME_SIZE;
             i += ENCODE_CODEC2_FRAME_SIZE) {
          // Decode the codec2 frame
          codec2_decode(codec2_state, output_buffer, rx_encode_frame + i);

          // Add to the audio buffer the 320 samples resulting of the decode of
          // the codec2 frame.
...
void setup() {
  ...
  axp.setPowerOutPut(AXP192_LDO2,
                     AXP202_ON); // provides power to GPS backup battery
  axp.setPowerOutPut(AXP192_LDO3, AXP202_ON);
  axp.setPowerOutPut(AXP192_DCDC2, AXP202_ON);
  axp.setPowerOutPut(AXP192_EXTEN, AXP202_ON);
  axp.setPowerOutPut(AXP192_DCDC1,
                     AXP202_ON); // enables power to ESP32 on T-beam
  axp.setPowerOutPut(AXP192_DCDC3,
                     AXP202_ON); // I foresee similar benefit for restting
                                 // T-watch where ESP32 is on DCDC3 but remember
                                 // to change I2C pins and GPS pins!
